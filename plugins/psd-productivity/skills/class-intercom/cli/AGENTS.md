# Classintercom Printed CLI Agent Guide

This directory is a generated `classintercom-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
classintercom-pp-cli doctor --json
classintercom-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
classintercom-pp-cli which "<capability>" --json
classintercom-pp-cli <command> --help
```
...
```bash
classintercom-pp-cli <command> --agent
```
...
```bash
classintercom-pp-cli <command> --help
classintercom-pp-cli <command> --dry-run --agent
```

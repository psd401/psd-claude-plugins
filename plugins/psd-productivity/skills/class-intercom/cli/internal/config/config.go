...
type Config struct {
	...
	ClassintercomCookies string            `toml:"cookies"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://app.classintercom.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("CLASSINTERCOM_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "classintercom-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("CLASSINTERCOM_COOKIES"); v != "" {
		cfg.ClassintercomCookies = v
		cfg.markEnvOverride("ClassintercomCookies")
		cfg.AuthSource = "env:CLASSINTERCOM_COOKIES"
	}
	...
	if cfg.AuthSource == "" && cfg.ClassintercomCookies != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("CLASSINTERCOM_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ClassintercomCookies != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CLASSINTERCOM_COOKIES"
		}
		return ensureAuthScheme("Bearer", c.ClassintercomCookies)
	}
	if c.AccessToken != "" {
		if c.AuthSource == "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.ClassintercomCookies = ""
	delete(c.envOverrides, "ClassintercomCookies")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ClassintercomCookies"] {
			out.ClassintercomCookies = c.fileConfig.ClassintercomCookies
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ClassintercomCookies":
		c.fileConfig.ClassintercomCookies = c.ClassintercomCookies
	}
}

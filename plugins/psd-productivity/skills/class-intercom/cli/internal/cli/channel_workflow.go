...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  classintercom-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  classintercom-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"content", "reports", "social", "tasks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  classintercom-pp-cli workflow status

  # Show status as JSON
  classintercom-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("classintercom-pp-cli")
			}

...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Classintercom",
...
Use --chrome to read cookies from Chrome for app.classintercom.com.
...
profile by name when the installed backend supports it.`,
		Example: `  classintercom-pp-cli auth login --chrome
  classintercom-pp-cli auth login --browser
  classintercom-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  classintercom-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  classintercom-pp-cli auth login --browser\n")
				return nil
			}
			// Check if already authenticated via env var
			if v := os.Getenv("CLASSINTERCOM_COOKIES"); v != "" {
				fmt.Fprintf(cmd.OutOrStdout(), "Already authenticated via %s env var.\n", "CLASSINTERCOM_COOKIES")
				return nil
			}

			w := cmd.OutOrStdout()
			domain := "app.classintercom.com"
...
func newAuthRefreshCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  classintercom-pp-cli auth refresh",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func browserSessionTargetURL(cfg *config.Config) string {
	baseURL := strings.TrimRight("https://app.classintercom.com", "/")
	if cfg != nil && cfg.BaseURL != "" {
		baseURL = strings.TrimRight(cfg.BaseURL, "/")
	}
	validationPath := strings.TrimSpace("/api/channels")
	if validationPath == "" {
		return baseURL
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  classintercom-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  classintercom-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: app.classintercom.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  classintercom-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  classintercom-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if os.Getenv("CLASSINTERCOM_COOKIES") != "" {
...
func browserSessionProofPath(cfg *config.Config) string {
	if cfg == nil || cfg.Path == "" {
		...
		return filepath.Join(home, ".config", "classintercom-pp-cli", "browser-session-proof.json")
	}
...
func browserSessionProofStatusForAuth(cfg *config.Config, authHeader string) (bool, string) {
	...
	if err != nil {
		return false, "proof not found; run classintercom-pp-cli auth login --chrome"
	}
	var proof browserSessionProof
	if err := json.Unmarshal(data, &proof); err != nil {
		return false, "proof is not valid JSON; re-run classintercom-pp-cli auth login --chrome"
	}
	if proof.APIName != "classintercom" || proof.CookieDomain != "app.classintercom.com" || proof.ValidationPath != "/api/channels" {
...
func validateAndWriteBrowserSessionProof(ctx context.Context, cfg *config.Config, flags *rootFlags) error {
	...
	proof := browserSessionProof{
		APIName:               "classintercom",
		CookieDomain:          "app.classintercom.com",

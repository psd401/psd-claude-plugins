...
In local mode: searches locally synced data only.`,
  ...
  classintercom-pp-cli search "error timeout"

  # Force local search only
  classintercom-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  classintercom-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  classintercom-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'classintercom-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "social":
				results, err = db.SearchSocial(query, limit)
			case "tasks":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchContent(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search content failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchSocial(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search social failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {

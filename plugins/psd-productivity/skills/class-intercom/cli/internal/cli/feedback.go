...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "classintercom-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLASSINTERCOM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLASSINTERCOM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "classintercom-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/classintercom-pp-cli/feedback.jsonl.
When ` + "`CLASSINTERCOM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLASSINTERCOM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  classintercom-pp-cli feedback list
  classintercom-pp-cli feedback list --limit 5
  classintercom-pp-cli feedback list --json`,

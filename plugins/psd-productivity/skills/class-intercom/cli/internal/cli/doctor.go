...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  classintercom-pp-cli doctor
  classintercom-pp-cli doctor --json
  classintercom-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "classintercom-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = "app.classintercom.com"
			...
			if os.Getenv("CLASSINTERCOM_COOKIES") != "" {
				authEnvSet = append(authEnvSet, "CLASSINTERCOM_COOKIES")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CLASSINTERCOM_COOKIES")
			}
			switch {
...
func browserSessionProofStatus(cfg *config.Config, authHeader string) (bool, string) {
	...
	if proof.APIName != "classintercom" {
		return false, "proof belongs to a different CLI"
	}
	if proof.CookieDomain != "app.classintercom.com" {
		return false, "proof belongs to a different cookie domain"
	}
	if proof.ValidationPath != "/api/channels" {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("classintercom-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'classintercom-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'classintercom-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'classintercom-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'classintercom-pp-cli sync' to refresh."
	}
	return report
}

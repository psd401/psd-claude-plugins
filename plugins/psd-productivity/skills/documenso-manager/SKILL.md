...
- **Webhooks are per-team, UI-only** — switching to a new team (or creating one) requires recreating the webhook subscription. No API to do this. Path: Documenso UI → team → Settings → Webhooks. URL: `https://<your-n8n-host>/webhook/documenso-completed`, event: `DOCUMENT_COMPLETED` or `ENVELOPE_COMPLETED` depending on version.

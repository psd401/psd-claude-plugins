# Parentsquare Printed CLI Agent Guide

This directory is a generated `parentsquare-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
parentsquare-pp-cli doctor --json
parentsquare-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
parentsquare-pp-cli which "<capability>" --json
parentsquare-pp-cli <command> --help
```
...
```bash
parentsquare-pp-cli <command> --agent
```
...
```bash
parentsquare-pp-cli <command> --help
parentsquare-pp-cli <command> --dry-run --agent
```

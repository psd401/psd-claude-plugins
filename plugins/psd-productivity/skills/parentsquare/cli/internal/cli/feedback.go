...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "parentsquare-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PARENTSQUARE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PARENTSQUARE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "parentsquare-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/parentsquare-pp-cli/feedback.jsonl.
When ` + "`PARENTSQUARE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PARENTSQUARE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  parentsquare-pp-cli feedback list
  parentsquare-pp-cli feedback list --limit 5
  parentsquare-pp-cli feedback list --json`,

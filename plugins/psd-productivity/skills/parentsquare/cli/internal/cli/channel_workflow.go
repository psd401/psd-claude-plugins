...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  parentsquare-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  parentsquare-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  parentsquare-pp-cli workflow status

  # Show status as JSON
  parentsquare-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("parentsquare-pp-cli")
			}

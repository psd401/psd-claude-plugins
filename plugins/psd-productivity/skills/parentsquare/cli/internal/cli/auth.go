...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Parentsquare",
...
Use --chrome to read cookies from Chrome for www.parentsquare.com.
...
profile by name when the installed backend supports it.`,
		Example: `  parentsquare-pp-cli auth login --chrome
  parentsquare-pp-cli auth login --browser
  parentsquare-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  parentsquare-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  parentsquare-pp-cli auth login --browser\n")
				return nil
			}
			// Check if already authenticated via env var
			if v := os.Getenv("PARENTSQUARE_COOKIES"); v != "" {
				fmt.Fprintf(cmd.OutOrStdout(), "Already authenticated via %s env var.\n", "PARENTSQUARE_COOKIES")
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  parentsquare-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  parentsquare-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: www.parentsquare.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  parentsquare-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  parentsquare-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)

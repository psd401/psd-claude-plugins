...
Resource scoping:
  ...
  parentsquare-pp-cli sync

  # Sync specific resources only
  parentsquare-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  parentsquare-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  parentsquare-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  parentsquare-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  parentsquare-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "parentsquare-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/parentsquare-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "districts":
		return db.UpsertDistricts(data)
	case "schools":
		return db.UpsertSchools(data)
	case "sections":

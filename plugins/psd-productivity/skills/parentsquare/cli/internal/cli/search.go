...
In local mode: searches locally synced data only.`,
  ...
  parentsquare-pp-cli search "error timeout"

  # Force local search only
  parentsquare-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  parentsquare-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  parentsquare-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'parentsquare-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "districts":
				results, err = db.SearchDistricts(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchDistricts(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search districts failed: %w", searchErr)
					}

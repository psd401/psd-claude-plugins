...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "parentsquare-pp-cli", "http")
...
// GetRaw issues an authenticated GET and returns the raw response body without
// JSON sanitization or caching. Hand-written commands use it to fetch HTML
// pages (e.g. server-rendered admin tables) for direct parsing.
func (c *Client) GetRaw(ctx context.Context, path string, params map[string]string) ([]byte, int, error) {
	...
	if resp.StatusCode >= 400 {
		return body, resp.StatusCode, fmt.Errorf("GET %s returned HTTP %d", path, resp.StatusCode)
	}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export PARENTSQUARE_COOKIES=<your-token> or parentsquare-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.ParentsquareCookies)
	}

...
type Config struct {
	...
	ParentsquareCookies string            `toml:"cookies"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://www.parentsquare.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("PARENTSQUARE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "parentsquare-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("PARENTSQUARE_COOKIES"); v != "" {
		cfg.ParentsquareCookies = v
		cfg.markEnvOverride("ParentsquareCookies")
		cfg.AuthSource = "env:PARENTSQUARE_COOKIES"
	}
	...
	if cfg.AuthSource == "" && cfg.ParentsquareCookies != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("PARENTSQUARE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ParentsquareCookies != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:PARENTSQUARE_COOKIES"
		}
		return ensureAuthScheme("Bearer", c.ParentsquareCookies)
	}
	if c.AccessToken != "" {
		if c.AuthSource == "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.ParentsquareCookies = ""
	delete(c.envOverrides, "ParentsquareCookies")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ParentsquareCookies"] {
			out.ParentsquareCookies = c.fileConfig.ParentsquareCookies
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ParentsquareCookies":
		c.fileConfig.ParentsquareCookies = c.ParentsquareCookies
	}
}

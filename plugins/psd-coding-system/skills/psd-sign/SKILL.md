...
- An app-specific password for that Apple ID (generate at https://appleid.apple.com), **or** a stored notarytool keychain profile (preferred — see Step 0)
...
2. **Credentials** — a notarytool keychain profile name if one exists (see Step 0); otherwise the `@psd401.net` Apple ID + app-specific password. SECURITY: never log, echo, or store the password
...
### Step 0 (one-time, optional but preferred): Store notarization credentials in the keychain

A stored profile keeps the app-specific password out of process listings and shell history. Offer this on first use; afterwards every notarytool call uses `--keychain-profile psd-notary` instead of `--apple-id/--password`.

```bash
xcrun notarytool store-credentials psd-notary \
  --apple-id "$APPLE_ID" \
  --team-id 87DL7L9GU6
# prompts interactively for the app-specific password — have the user run this themselves
```

If the user declines or is on a one-off machine, fall back to passing `--apple-id "$APPLE_ID" --team-id 87DL7L9GU6 --password "$APP_PASSWORD"` in Steps 4 and 7.
...
- **Notarization rejected** — Check the log with `xcrun notarytool log <submission-id> --keychain-profile psd-notary` (or the `--apple-id/--team-id/--password` flags)
